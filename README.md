...
This builds on an earlier prototype (**Voltiva 1.0**) developed the previous semester, which established basic voltage/current/power measurement with a simple web interface. This semester's work extended it with Telegram bot alerts, a Blynk 2.0 dashboard, session-based tracking, sensor calibration, and AC/DC isolation for safety.
...
> ⚠️ **Never commit real credentials.** Keep them in a separate untracked `secrets.h` file (added to `.gitignore`) rather than hard-coded in the main sketch.
